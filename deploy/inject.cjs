...
const parts = [
	{
		name: 'пауза до языка',
		// Метка — по комментарию, а не по имени атрибута: атрибут встречается ещё
		// и в правиле стилей, которое лежит в бандле и попадает в каждый снимок.
		// По нему шаг решал, что скрипт уже на месте, и не вставлял его никуда.
		mark: 'Язык приезжает позже страницы',
		before: '</head>',
