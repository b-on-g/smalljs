...
[Giper Baza](https://github.com/giper-dev/baza) は**独立した、任意の（オプションの）プロジェクト**であり、$mol の組み込みの一部ではありません。作者はこれを*競合のないリアルタイム同期を備えた分散型・高可用性データベース*と説明しています。中央サーバーなしでローカルに永続化し、クライアント間で複製する CRDT ストアで、電子署名とエンドツーエンド暗号化を備えます。$mol アプリを作るのに必須ではありません。複数のクライアントやデバイスが同じライブデータを共有する必要があるときにだけ使ってください。

> アプリをネットワークなしで動かし続けたいだけですか？ それは純粋なオフラインで、$mol は service worker で対応します——[オフライン](#!section=docs/page=offline)を参照してください。Giper Baza はその一歩先です。1 つのクライアントのアセットをキャッシュするのではなく、クライアント*間*でデータを同期します。

実際にこれでデータをモデル化すると、エンティティはふつうのリアクティブなプロパティのように見え、複製はただ起こります。
