...
より深い違いは上の語彙ではなく、繰り返し現れる問題を*誰*が解くかにあります。React、Vue、Svelte では、その多くはすべてのコンポーネントで再実装し、それぞれのやり方で壊してしまうパターンです。$mol ではそれらは一つのメカニズムの性質なので、より使いやすい API を得るのではなく、その種の作業ごと消えてなくなります。

- **状態ストア** — React の 5 つのストレージ・サブシステム（Redux + RTK Query + `useState` + `useReducer` + Context）が一つにまとまります。コンポーネント上のリアクティブなフィールドです。
- **リアクティビティ** — 他所では後付け（MobX、signals）ですが、ここでは土台にあるので、何もオプトインする必要がありません。
- **クラッシュの分離** — 手で置く `<ErrorBoundary>` に対し、すべてのコンポーネントがデフォルトで自分自身の境界になります。
- **副作用** — 十数個のシナリオにまたがる `useEffect`/`useMemo`/deps のチェックリストに対し、一つのメソッド（あるいは action を持つメソッド）。ツリー上の分岐は一つで、十二個ではありません。
- **仮想化** — リストごとの第三者コンポーネントに対し、ビューポートの外にあるものはそもそも存在しません。
