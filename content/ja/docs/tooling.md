...
翻訳者が欲しいのはファイル 1 つであって、30 個ではありません。ビルド済みのアプリにはすでにそれがあります。`<app>/-/web.locale=<lang>.json` には、そのアプリがバンドルする全モジュールの文字列がすべて入っています。これを渡して翻訳してもらい、戻ってきたらモジュールごとに振り分けます。
...
キーは自分のモジュールパスを含んでいるので、`$my_page_greeting` は所属するソースの隣、`my/page/page.locale=<lang>.json` に収まります。引数はフォルダでも、単一のロケールファイルでも構いません。

- `--include=<パスの一部>` — パスにその断片を含むモジュールだけを対象にする。複数指定可
- `--exclude=<パスの一部>` — それらを飛ばす。`--exclude=mol` ならフレームワーク自身のパッケージには手を触れない
- `--update` — 既存ファイルにマージする。入力側の値が勝ち、入力に無いキーはそのまま残る
- `--dry` — 計画を表示するだけで、何も書き込まない

キーからモジュールを特定するのは見た目より厄介です。`_` はフォルダの区切りでも単語の区切りでもあるため、「最長一致するパス」は答えになりません。`$my_page_lang_hint` ではプロパティが `lang` で始まり、隣に本物の `my/page/lang` サブモジュールがあれば、そちらがキーを飲み込んでしまいます。そこでこのコマンドは、候補となる各モジュールに「どのキーを宣言しているか」を尋ねます。MAM はまさにそれを `<module>/-view.tree/*.locale=en.json` に書き出しているので、キーは正しい持ち主に渡ります。
