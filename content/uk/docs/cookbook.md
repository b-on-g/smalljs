...
Асинхронне значення — це просто реактивна властивість, написана так, ніби відповідь уже є. `$mol_fetch` призупиняє волокно, доки запит у дорозі, і властивість повертає розібрані дані, щойно вони приземлилися, тож будь-яке подання, що читає його, показує вбудований стан завантаження — а невдалий запит зринає як стан помилки. Ви не пишете ні прапорця `isLoading`, ні `try`/`catch`. Виклик іде через `this.$`, контекст компонента, тож [тест](#!section=docs/page=testing) може підмінити мережу моком.
