...
## Розкладання перекладів

Перекладачеві потрібен один файл, а не тридцять. У зібраного застосунку такий файл уже є: у `<app>/-/web.locale=<lang>.json` лежать усі рядки всіх модулів, які він містить. Віддайте його, отримайте назад перекладеним і розкладіть по модулях:
...
Ключ несе шлях до свого модуля, тому `$my_page_greeting` потрапить у `my/page/page.locale=<lang>.json` — поруч із джерелами, яким належить. Аргументом можна дати і теку, і окремий файл локалі.

- `--include=<частина шляху>` — лише модулі, чий шлях містить фрагмент; можна вказувати кілька разів
- `--exclude=<частина шляху>` — пропустити такі; `--exclude=mol` залишить пакети самого фреймворка недоторканими
- `--update` — дописати в наявні файли: значення з джерела перемагають, ключі, яких у ньому немає, лишаються
- `--dry` — показати план і нічого не записувати

Визначити модуль за ключем складніше, ніж здається. `_` розділяє і теки, і слова, тому «найдовший збіг шляху» — хибна відповідь: у `$my_page_lang_hint` властивість починається на `lang`, і справжній сусідній підмодуль `my/page/lang` забрав би ключ собі. Тому команда питає в кожного кандидата, які ключі він оголосив — MAM записує саме їх у `<модуль>/-view.tree/*.locale=en.json`, — і віддає ключ тому, хто ним володіє.
