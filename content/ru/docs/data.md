...
Позовите сеть внутри `@ $mol_mem` и верните разобранный результат. Ни промиса, ни `await`, ни колбэка в коде нет:
...
`$mol_fetch` приостанавливает фибру, пока не придёт ответ, после чего вычисление перезапускается и `users()` возвращает массив. Пока запрос в ожидании, любое вью, читающее `users()`, автоматически показывает встроенное состояние загрузки — вы не пишете флаг `isLoading`. Сами промис из `@ $mol_mem` не возвращайте: положенный в значение промис оставляет ячейку в состоянии загрузки навсегда, см. [Решение проблем](#!section=docs/page=troubleshooting).

`this.$` — это контекст компонента. Каждый сервис приходит через него: `$mol_fetch` для сети, `$mol_state_arg` для URL, `$mol_after_timeout` для времени. В тесте контекст подменяется, поэтому тот же `users()` отправляет свой запрос в мок вместо сети, и в компоненте не меняется ни строчки. Этот мок показывает [Тестирование](#!section=docs/page=testing).
...
Когда приходит ответ, `users()` обновляется, `user_names()` пересчитывается, и список рендерится. Ни колбэков, ни `useEffect`.
