...
Асинхронное значение — это просто реактивное свойство, написанное так, будто ответ уже есть. `$mol_fetch` приостанавливает фибру, пока запрос в пути, и свойство возвращает разобранные данные, как только они приземлились, поэтому любое вью, читающее его, показывает встроенное состояние загрузки — а неудавшийся запрос всплывает как состояние ошибки. Вы не пишете ни флага `isLoading`, ни `try`/`catch`. Вызов идёт через `this.$`, контекст компонента, поэтому [тест](#!section=docs/page=testing) может подменить сеть моком.
