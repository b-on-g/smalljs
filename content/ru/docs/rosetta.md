...
| Загрузка данных при монтировании | `useEffect(fetch)` | `onMounted(fetch)` | `onMount(fetch)` | хука нет: читайте данные в свойстве, вью запросит их, когда отрендерится |
...
Когда строка таблицы переименовывает то, на что вы опирались, [Ментальная модель](#!section=docs/page=mental-model) объясняет идею за новым именем и почему того хука, который вы ищете, здесь нет.
