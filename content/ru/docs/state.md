...
`@ $mol_mem`, которому нужны данные из сети, зовёт её через контекст так, будто ответ уже есть. Фибра приостанавливается, пока не придёт ответ, вью тем временем показывает состояние загрузки, и ни флага загрузки, ни промиса в вашем коде нет:
...
Не делайте метод `async` и не возвращайте из него промис: промис, оставленный значением, навсегда оставляет ячейку вычисляющейся. [Загрузка данных](#!section=docs/page=data) развивает этот паттерн.
