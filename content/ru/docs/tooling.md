...
Переводчику нужен один файл, а не тридцать. У собранного приложения такой файл уже есть: в `<app>/-/web.locale=<lang>.json` лежат все строки всех модулей, которые оно включает. Отдайте его, получите обратно переведённым и разложите по модулям:
...
Ключ несёт путь к своему модулю, поэтому `$my_page_greeting` попадёт в `my/page/page.locale=<lang>.json` — рядом с исходниками, которым принадлежит. Аргументом можно дать и папку, и отдельный файл локали.

- `--include=<кусок пути>` — только модули, чей путь содержит фрагмент; можно указывать несколько раз
- `--exclude=<кусок пути>` — пропустить такие; `--exclude=mol` оставит пакеты самого фреймворка нетронутыми
- `--update` — дописать в существующие файлы: значения из источника побеждают, ключи, которых в нём нет, остаются
- `--dry` — показать план и ничего не записывать

Определить модуль по ключу сложнее, чем кажется. `_` разделяет и папки, и слова, поэтому «самый длинный совпавший путь» — неверный ответ: в `$my_page_lang_hint` свойство начинается на `lang`, и настоящий соседний подмодуль `my/page/lang` забрал бы ключ себе. Поэтому команда спрашивает у каждого кандидата, какие ключи он объявил — MAM записывает ровно их в `<модуль>/-view.tree/*.locale=en.json`, — и отдаёт ключ тому, кто им владеет.
