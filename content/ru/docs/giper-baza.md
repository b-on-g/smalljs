...
[Гипер База](https://github.com/giper-dev/baza) — это **отдельный, необязательный проект**, а не встроенная часть $mol. Авторы описывают его как *децентрализованную высокодоступную базу данных с бесконфликтной синхронизацией в реальном времени*: CRDT-хранилище, которое сохраняется локально и реплицируется между клиентами без центрального сервера, с цифровыми подписями и сквозным шифрованием. Для приложения на $mol он никогда не нужен; берите его только тогда, когда нескольким клиентам или устройствам нужно делить одни и те же живые данные.

> Просто хотите, чтобы приложение продолжало работать без сети? Это обычный офлайн, и $mol обеспечивает его через service worker — см. [Офлайн](#!section=docs/page=offline). Гипер База — следующий шаг: синхронизация данных *между* клиентами, а не кэширование ресурсов одного клиента.

Когда вы действительно моделируете с ней данные, сущности выглядят как обычные реактивные свойства, а репликация просто происходит.
