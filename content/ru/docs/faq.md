...
Небольшой. Минимальное приложение — это около 123 KB несжатого JavaScript, или примерно 20 KB по сети после сжатия. Рендеринг виртуализирован по умолчанию (компоненты за пределами вьюпорта никогда не создаются), а сборка включает только те модули, которые вы реально используете, поэтому бандл растёт вместе с вашим приложением, а не с фреймворком. Подробности и воспроизводимые бенчмарки — в [Рендеринге](#!section=docs/page=rendering).
