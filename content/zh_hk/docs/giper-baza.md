...
[Giper Baza](https://github.com/giper-dev/baza) 是一個**獨立的、可選的專案**——並不是 $mol 的內建組成部分。其作者將它描述為*一個具有無衝突即時同步的去中心化高可用資料庫*：一個在本地持久化、無需中心伺服器即可在用戶端之間複製的 CRDT 儲存，帶有數位簽章和端到端加密。建構 $mol 應用從不需要它；只有當多個用戶端或裝置必須共用同一份即時資料時才使用它。

> 只是想讓應用在沒有網絡時也能繼續運作？那是純粹的離線，$mol 用一個 service worker 來處理——參見 [離線](#!section=docs/page=offline)。Giper Baza 更進一步：在用戶端*之間*同步資料，而不是快取單個用戶端的資源。

當你確實用它來建模資料時，實體看起來就像普通的響應式屬性，而複製自然而然地發生。
