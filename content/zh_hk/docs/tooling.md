...
譯者要的是一個檔案，而不是三十個。建置好的應用裡已經有了：`<app>/-/web.locale=<lang>.json` 匯集了該應用打包的所有模組的全部文案。把它送出去，取回翻譯好的版本，再拆回各個模組：
...
每個鍵本身帶著所屬模組的路徑，所以 `$my_page_greeting` 會落到 `my/page/page.locale=<lang>.json`，就在它所屬的原始碼旁邊。參數可以是資料夾，也可以是單一語言檔案。

- `--include=<路徑片段>` — 只處理路徑包含該片段的模組；可重複指定
- `--exclude=<路徑片段>` — 略過這些模組；`--exclude=mol` 可讓框架本身的套件維持原樣
- `--update` — 合併進既有檔案：來源中的值優先，來源中沒有的鍵保持不動
- `--dry` — 只印出計畫，不寫入任何檔案

依鍵名判斷模組比看起來更微妙。`_` 既分隔資料夾也分隔字詞，所以「最長相符路徑」並非答案：`$my_page_lang_hint` 的屬性名以 `lang` 開頭，若旁邊真的存在 `my/page/lang` 子模組，這個鍵就會被它吞掉。因此指令會逐一詢問候選模組宣告了哪些鍵——MAM 正是把這些鍵寫進 `<模組>/-view.tree/*.locale=en.json`——再把鍵交給真正的擁有者。
