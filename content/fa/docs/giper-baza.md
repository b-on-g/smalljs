...
[Giper Baza](https://github.com/giper-dev/baza) یک **پروژهٔ جداگانه و اختیاری** است — نه بخشی توکار از $mol. سازندگانش آن را *یک پایگاه‌دادهٔ غیرمتمرکز و پُردسترس با همگام‌سازیِ بی‌درگیریِ بی‌درنگ* توصیف می‌کنند: یک انبارِ CRDT که به‌صورت محلی پایدار می‌ماند و بدون سرورِ مرکزی میان کلاینت‌ها تکثیر می‌شود، همراه با امضای دیجیتال و رمزنگاریِ سرتاسری. برای ساختِ یک اپِ $mol هرگز به آن نیاز ندارید؛ تنها زمانی سراغش بروید که چند کلاینت یا دستگاه باید دادهٔ زندهٔ یکسانی را به‌اشتراک بگذارند.

> فقط می‌خواهید اپ‌تان بدون شبکه هم کار کند؟ آن آفلاینِ ساده است و $mol آن را با یک service worker مدیریت می‌کند — [آفلاین](#!section=docs/page=offline) را ببینید. Giper Baza یک گام جلوتر است: همگام‌سازیِ داده *میانِ* کلاینت‌ها، نه کش‌کردنِ داراییِ یک کلاینت.

وقتی واقعاً با آن داده را مدل می‌کنید، موجودیت‌ها مانند ویژگی‌های واکنش‌گرای معمولی به نظر می‌رسند، و تکثیر خودبه‌خود رخ می‌دهد.
