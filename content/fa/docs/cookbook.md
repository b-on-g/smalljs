...
مقدارِ ناهمگام فقط یک ویژگیِ واکنش‌گراست که چنان نوشته شده که انگار پاسخ از پیش آنجاست. `$mol_fetch` تا زمانی که درخواست در راه است فایبر را معلق می‌کند و ویژگی به‌محضِ رسیدنِ داده مقدارِ پارس‌شده را برمی‌گرداند، بنابراین هر نمایی که آن را می‌خواند وضعیتِ بارگذاریِ توکار را نشان می‌دهد — و یک درخواستِ ناموفق به‌صورتِ وضعیتِ خطا رو می‌آید. نه پرچمِ `isLoading` می‌نویسید و نه `try`/`catch`. فراخوانی از راهِ `this.$` می‌گذرد، یعنی زمینهٔ کامپوننت، پس یک [تست](#!section=docs/page=testing) می‌تواند شبکه را با یک ماک عوض کند.
