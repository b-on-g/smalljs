...
آن را از ریشهٔ کپی MAM خود اجرا کنید: مسیرهای ماژول از همان‌جا حساب می‌شوند و پروژه هم باید همان‌جا باشد. بیرون از ورک‌اسپیس، دستور هشدار می‌دهد تا در اولین بیلد غافلگیر نشوید.

آرگومان همان مسیر ماژول است (`namespace/name` یا معادل آن `bog_myapp`). این دستور `view.tree`، `view.ts`، `view.css.ts` و `index.html` یک برنامهٔ کارآمد را می‌نویسد، به‌علاوهٔ GitHub Actions برای استقرار آن.

هر چیزی که سازندهٔ پروژه بلد است اضافه کند، به‌طور پیش‌فرض هست. شما فقط چیزی را نام می‌برید که نمی‌خواهید:

```bash
npx create-view-tree-lsp bog/myapp --no-tauri --no-backend
```

- `--no-baza` — ذخیره‌گاه local-first به نام **Giper Baza**
- `--no-docker` — پیکربندی **Docker** همراه `docker-compose.yml` و کانفیگ nginx
- `--no-tauri` — پوستهٔ دسکتاپ **Tauri**
- `--no-backend` — بک‌اند REST به نام `$mol_server` با ذخیره‌سازی `node:sqlite` و یک نوع آیتم TypeScript مشترک
- `--no-prerender`، `--no-seo` — دیده‌شدن در موتورهای جست‌وجو، که در پایین زیر بخش [یکپارچه‌سازی مداوم](#!section=docs/page=tooling/Docs.Body=%DB%8C%DA%A9%D9%BE%D8%A7%D8%B1%DA%86%D9%87%E2%80%8C%D8%B3%D8%A7%D8%B2%DB%8C%20%D9%85%D8%AF%D8%A7%D9%88%D9%85) توضیح داده شده است

پرچم ناشناخته اجرا را متوقف می‌کند تا یک غلط تایپی چیزی را بی‌سروصدا جا نگذارد.
...
ترجمه‌ها کنار ماژول خودشان می‌نشینند، در `<module>/<name>.locale=<lang>.json`. برای کد راحت است، برای مترجم نه: به‌جای یک فهرست از عبارت‌ها، سی فایل ریز تحویل می‌گیرد.

**[$yuf_localizer](https://zerkalica.github.io/yuf/#!demo=yuf_localizer_demo)** همین شکاف را پر می‌کند. نشانی پروژه‌ها و کدهای زبان را به آن بدهید تا همهٔ کلیدها را در یک فهرستِ قابل‌جست‌وجو نشان دهد و آنچه هنوز مانده را علامت بزند: کلیدهایی که فقط انگلیسی دارند، کلیدهایی که تغییر داده‌اید ولی ثبت نکرده‌اید، و کلیدهای کهنه‌ای که پروژه دیگر ندارد. ترجمه‌ها تا زمان برون‌ریزی در مرورگر می‌مانند، پس میان دو نشست چیزی گم نمی‌شود.

وقتی کار مترجم تمام شد، نتیجه را برون‌ریزی کنید و دوباره میان ماژول‌ها پخش کنید:
...
آرگومان می‌تواند یک پوشه باشد یا یک فایل زبان. گزینه‌ها:

- `--include=` تکه‌ای از مسیر می‌گیرد و تنها ماژول‌هایی را نگه می‌دارد که مسیرشان آن را دارد؛ هر چند بار که بخواهید تکرارشدنی است
- `--exclude=` برعکس، از آن‌ها می‌گذرد — `--exclude=mol` بسته‌های خودِ فریم‌ورک را دست‌نخورده می‌گذارد
- `--update` با فایل‌های موجود ادغام می‌کند: مقدارهای ورودی برنده‌اند و کلیدهایی که در ورودی نیستند می‌مانند
- `--dry` نقشهٔ کار را چاپ می‌کند و چیزی نمی‌نویسد

هر کلید مسیر ماژول خودش را با خود دارد، بنابراین `$my_page_greeting` در `my/page/page.locale=ru.json` می‌نشیند، کنار همان منبعی که به آن تعلق دارد. اما یافتن آن ماژول ظریف‌تر از چیزی است که به نظر می‌رسد: `_` هم پوشه‌ها را جدا می‌کند و هم واژه‌ها را، پس «طولانی‌ترین مسیرِ منطبق» پاسخ نادرستی است. در `$my_page_lang_hint` نام ویژگی با `lang` آغاز می‌شود و اگر واقعاً زیرماژولی به نام `my/page/lang` کنارش باشد، کلید را می‌بلعد. برای همین فرمان از هر ماژولِ نامزد می‌پرسد چه کلیدهایی را اعلام کرده است — MAM دقیقاً همان‌ها را در فایل زبانِ `-view.tree` آن ماژول می‌نویسد — و کلید را به صاحب واقعی‌اش می‌سپارد.
...
هر دو به‌طور پیش‌فرض روشن‌اند و هر دو روی تگ‌های `v*` اجرا می‌شوند:

- **`--no-prerender`** گامی را حذف می‌کند که صفحه‌هایی را که فهرست کرده‌اید (مثل `home`) با `b-on-g/mol-prerender-action` به HTML ایستا رندر می‌کند — همان چیزی که باعث می‌شود خزنده‌ها و پیش‌نمایش لینک‌ها محتوای واقعی ببینند.
- **`--no-seo`** رانتایم `$bog_seo` را حذف می‌کند: مسیریاب بر پایهٔ pathname همراه نقشهٔ سایت، `robots.txt`، `llms.txt` و تزریق متا برای هر صفحه. این کار بیلد را سرو می‌کند، HTML پیش‌رندرشدهٔ کانونی را بیرون می‌ریزد و دوباره در استقرار جا می‌دهد.

هر دو یک کار می‌کنند و در یک پوشه می‌نویسند، پس فقط یکی به `deploy.yml` می‌رسد: تا وقتی `$bog_seo` روشن است خودش، و به‌محض دادن `--no-seo` نوبت prerender-action است. وقتی نقشهٔ سایت و متای هر صفحه لازم دارید `$bog_seo` را نگه دارید، و وقتی همهٔ کار چند صفحهٔ عمومی است به prerender-action بسنده کنید.
