...
## অনুবাদ মডিউলে ভাগ করা

অনুবাদক একটি ফাইল চান, ত্রিশটি নয়। বিল্ড করা অ্যাপে সেটি আগে থেকেই আছে: `<app>/-/web.locale=<lang>.json`-এ অ্যাপ যত মডিউল বান্ডল করে তাদের সব লেখা এক জায়গায় থাকে। ওটাই পাঠান, অনুবাদ হয়ে ফিরলে আবার মডিউল অনুযায়ী ভাগ করে নিন:
...
প্রতিটি কী নিজের মডিউলের পথ বহন করে, তাই `$my_page_greeting` গিয়ে বসে `my/page/page.locale=<lang>.json`-এ — ঠিক যে সোর্সের সঙ্গে তার সম্পর্ক, তার পাশেই। আর্গুমেন্ট হিসেবে ফোল্ডার বা একটিমাত্র লোকেল ফাইল, দুটোই চলে।

- `--include=<পথের অংশ>` — কেবল সেই মডিউল যাদের পথে ওই অংশটি আছে; একাধিকবার দেওয়া যায়
- `--exclude=<পথের অংশ>` — সেগুলো বাদ দিন; `--exclude=mol` দিলে ফ্রেমওয়ার্কের নিজের প্যাকেজে হাত পড়ে না
- `--update` — বিদ্যমান ফাইলের সঙ্গে মেলান: উৎসের মান জেতে, উৎসে না থাকা কী-গুলো থেকে যায়
- `--dry` — পরিকল্পনা দেখান, কিছুই লিখবেন না

কী দেখে মডিউল বের করা দেখতে যতটা সহজ, তত নয়। `_` ফোল্ডার আর শব্দ — দুটোই আলাদা করে, তাই "সবচেয়ে লম্বা মিলে যাওয়া পথ" উত্তর নয়: `$my_page_lang_hint`-এ প্রপার্টির নাম শুরু হয় `lang` দিয়ে, আর পাশে সত্যিই `my/page/lang` সাবমডিউল থাকলে সে-ই কী-টি গিলে ফেলবে। তাই কমান্ডটি প্রতিটি সম্ভাব্য মডিউলকে জিজ্ঞাসা করে সে কোন কী ঘোষণা করেছে — MAM ঠিক সেগুলোই `<module>/-view.tree/*.locale=en.json`-এ লেখে — এবং কী-টি তার প্রকৃত মালিককে দেয়।
